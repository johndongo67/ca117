...
class Student(object):
    ...
    def __str__(self):
        ...
        results = '\n'.join([code + ' : ' + self.mods[code].title +
                            ' : ' + str(self.marks[code]) for code in sorted(self.mods.keys())])
    ...
    def add_mark(self, name, marks):
        self.marks[name] = marks

    def precision_mark(self):
        weighted_marks = [self.mods[code].ects * self.marks[code] for code in self.mods]
        return sum(weighted_marks) / sum([self.mods[code].ects for code in self.mods])
    ...
    def passed_by_compensation(self):
        ...
        total_credits = sum([self.mods[code].ects for code in self.mods])
        credits_failed = sum([self.mods[code].ects
